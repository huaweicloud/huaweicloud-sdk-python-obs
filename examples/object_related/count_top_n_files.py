...
"""
  This sample demonstrates how to list incremental objects within a specified time range in an OBS bucket using the OBS SDK for Python.
  The script uses pagination to efficiently handle large numbers of objects and filters the objects based on their last modified time.
"""
        ...
        while True:
            ...
            if resp.status < 300:
                ...
                incremental_objects.extend([
                    ...
                    if start_time <= datetime.datetime.strptime(obj.lastModified, '%Y-%m-%dT%H:%M:%S.%fZ') <= end_time
                ])
