...
if __name__ == '__main__':
    ...
    try:
        # Store all objects with public read permission.
        public_objects = []

        try:
            print(f"\nRetrieving the object list for the bucket {bucketName}....")
            max_num = 1000
            mark = None
            index = 1
            while True:
                ...
                if resp.status < 300:
                    ...
                    for content in resp.body.contents:
                        ...
                        try:
                            ...
                            acl_resp = obsClient.getObjectAcl(bucketName, content.key)
                            if acl_resp.status < 300:
                                ...
                                for grant in acl_resp.body.grants:
                                    ...
                                    if grant.grantee.group == "Everyone" and grant.permission == "READ":
                                        public_objects.append(f"{bucketName}/{content.key}")
                                        print(f"Found an object with public read permission: {bucketName}/{content.key}")
                            else:
            ...
            print(f'Failed to get the object list for the bucket {bucketName}')
            print('Error:', str(e))
            print('Traceback:', traceback.format_exc())
